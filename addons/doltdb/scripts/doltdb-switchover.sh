...
: "${DOLT_SWITCHOVER_WAIT_SECONDS:=60}"
...
wait_for_role() {
  ...
  expected_role="$2"
  min_epoch="$3"
  deadline="$(( $(date +%s) + DOLT_SWITCHOVER_WAIT_SECONDS ))"
...
run_switchover() {
  ...
  log "Current pod role=${current_role} epoch=${current_epoch}; candidate ${candidate_fqdn} role=${candidate_role} epoch=${candidate_epoch}"
